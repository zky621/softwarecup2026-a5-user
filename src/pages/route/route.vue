...
import { getRoutes } from '@/api/scenic'
import type { RouteItem } from '@/api/scenic'
...
<template>
  <view class="route-page min-h-screen bg-[#f4f7f3] pb-8">
    <view class="flex items-center gap-3 px-4 pb-2 pt-4">
    ...
    <view v-else class="px-4 space-y-3">
      <view v-for="route in routes" :key="route.id" class="route-card" :class="{ expanded: expandedId === route.id }">
        <view class="p-4" @click="toggleExpand(route.id)">
          ...
          <view class="mt-2 flex gap-4 text-12px text-[#66756f]">
        ...
        <view v-if="expandedId === route.id && route.stops?.length" class="border-t border-[#edf1ee] px-4 py-3">
          <view v-for="(stop, idx) in route.stops" :key="stop.spotName" class="mb-3 flex gap-3">
            <view class="w-6 flex flex-col items-center">
              <view class="h-5 w-5 flex items-center justify-center rounded-full bg-[#1f6d58] text-11px text-white font-700">
                {{ idx + 1 }}
              </view>
              <view v-if="idx < route.stops.length - 1" class="mt-1 w-0.5 flex-1 bg-[#dfe9e4]" />
            </view>
            <view class="min-w-0 flex-1 pb-2">
              <view class="text-15px text-[#20372f] font-700">
                {{ stop.spotName }}
              </view>
              <view v-if="stop.stayDuration" class="text-11px text-[#1f6d58]">
                {{ stop.stayDuration }}
              </view>
              <view v-if="stop.description" class="text-12px text-[#66756f]">
                {{ stop.description }}
              </view>
            </view>
          </view>
...
.route-card.expanded {
  box-shadow: 0 4px 16px rgba(29, 54, 46, 0.1);
}
.route-badge {
  display: flex;
  align-items: center;
